;; Copyright (c) James Reeves, David Santiago. All rights reserved.
...
(defvar *warn-on-interpret* false
...
(defn- compile-tag-2-ambiguous
  ...
  (let [rendered-tag-attrs (make-attrs tag-attrs)]
    ...
    (emit `(cond
            ...
            (str (#'make-attrs (merge ~tag-attrs ~@attrs-or-content))
                 (if ~standalone? " />" ">"))
  ...
  (if (can-compile-vector? element)
    (let [[tag attrs content] (parse-element element)]
      (if (and (= (count element) 2)
        ...
        (if (or content (container-tags tag))
          (do (emit "<" tag (compile-attrs attrs) ">")
  ...
  (cond
    ...
    (seq? data)    (if (or (vector? (first data))
                     ...
                     (do (perf-warning data)
                         (emit `(apply str (map #'render-html (vector ~@data))))))
